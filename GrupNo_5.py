...
============================================================
  ...
  GÜNCEL: PM Raporu Entegrasyonu (Decimal Koruması,
  Inline Error Handling, Logaritmik Grafik Ölçeği)
============================================================
"""
...
class Tooltip:
    ...
    def show_tip(self, event=None):
        ...
        label = tk.Label(tw, text=self.text, justify="left",
                         background=self.r["ENTRY_BG"], foreground=self.r["TEXT_PRIMARY"],
...
# ══════════════════════════════════════════════════════════════════════
#  MATEMATİK KATMANI (Decimal Koruması Eklendi)
# ══════════════════════════════════════════════════════════════════════
...
def hesapla_mevduat(baslangic, yillik_faiz, aylik_yatirim, donem_sayisi, stopaj_orani, vade_gun, baslangic_tarihi,
    ...
    SC = Decimal('1') - (Decimal(str(stopaj_orani)) / Decimal('100'))
    ...
    bakiyeler_nominal = [float(A0)]  # Çizim motoru için float kopya
    ...
    for n in range(donem_sayisi):
        ...
        # Float dönüşümleri kaldırıldı, saf Decimal nesneleri tabloya iletiliyor
        ekstre.append({
            "donem": donem_no, "tarih": aktif_tarih.strftime("%d.%m.%Y"),
            "faiz": net_faiz, "islem": guncel_islem,
            "bakiye": bakiye, "reel_bakiye": para_format(reel_bakiye),
            "vade_bozuldu": donem_vade_bozuldu
        })
    ...
    return {
        "nominal": bakiyeler_nominal, "reel": bakiyeler_reel, "p_min": p_min,
        "bitis_tarihi": aktif_tarih, "vade_bozuldu": vade_bozuldu,
        "vade_bozulan_donemler": vade_bozulan_donemler, "yanan_faiz": yanan_faiz_toplami,
        "ekstre": ekstre, "karakteristik": karakteristik
    }
...
def hesapla_borc(baslangic, yillik_faiz, aylik_odeme, vergi_orani, baslangic_tarihi, ara_odemeler=None,
    ...
    if P <= p_min and not ara_odemeler and hedef_vade is None:
        iraksar = True
        dongu_limiti = 24
    ...
    while n < dongu_limiti and borc > Decimal('0'):
        ...
        # Saf Decimal nesneleri tabloya iletiliyor
        ekstre.append({
            "taksit": n + 1, "tarih": aktif_tarih.strftime("%d.%m.%Y"),
            "kalan_borc": borc, "faiz": donem_faizi,
    ...
    return {
        "nominal": bakiyeler_nom, "iraksar": iraksar, "kapanma_ay": n, "p_min": p_min,
...
# ══════════════════════════════════════════════════════════════════════
#  GRAFİK KATMANI (Logaritmik Ölçek Desteği Eklendi)
# ══════════════════════════════════════════════════════════════════════
...
def ciz_grafik(canvas: tk.Canvas, veriler_nom: list, veriler_reel: list, renk_nom: str, baslik: str, r: dict,
    ...
    # Iraksak senaryolar için logaritmik hesaplama
    kullan_log = log_olcek and v_max > 0
    if kullan_log:
        v_min_log = 0
        v_max_log = math.log10(v_max) if v_max > 1 else 1
    ...
    def veri2pix(idx, val):
        ...
        if kullan_log:
            ...
            py = pad_top + ih - int((val_l - v_min_log) / (v_max_log - v_min_log) * ih)
        else:
    ...
    def cizgi_olustur(veriler, renk, kalinlik, is_dashed=False):
        ...
        if len(noktalar) >= 4:
            canvas.create_line(noktalar, fill=renk, width=kalinlik, smooth=False, dash=(4, 4) if is_dashed else None)
    ...
    def on_hover(event):
        ...
        if x < pad_left or x > pad_left + iw:
            on_leave(event);
            return
        ...
        if bbox:
            canvas.coords(tooltip_bg, bbox[0] - 6, bbox[1] - 6, bbox[2] + 6, bbox[3] + 6)
        ...
        if tree is not None and idx > 0:
            ...
            if cocuklar and idx <= len(cocuklar):
                ...
                if vurgulu_nokta[0] != hedef:
                    vurgulu_nokta[0] = hedef
                    tree.selection_set(hedef);
                    tree.see(hedef)
...
class UygulamaGUI:
    def __init__(self, kok: tk.Tk):
        ...
        self.b_vergi = tk.StringVar(value="")
    ...
    def _ikon_olustur(self, parent, tooltip_text):
        ...
        icon_lbl.tooltip = Tooltip(icon_lbl, tooltip_text, self.renkler)
        return icon_lbl
    ...
    def _genel_dogrulama(self, P, tip):
        ...
        if tip == "para_gorsel": return P.replace('.', '').count(',') <= 1 and all(
            c in "0123456789," for c in P.replace('.', '')) and (
                ',' not in P.replace('.', '') or len(P.replace('.', '').split(',')[1]) <= 2)
        return True
    ...
    def _dinamik_min_odeme(self, *args):
        try:
            ...
            if not b_bas_str: raise ValueError
            B0 = Decimal(b_bas_str)
            F = Decimal(self.b_faiz.get()) / Decimal('100');
            vergi_orani_d = Decimal(self.b_vergi.get()) / Decimal('100')
            r_aylik = (F / Decimal('12')) * (Decimal('1') + vergi_orani_d)
            self.lbl_dinamik_min.config(
                text=f"Tahakkuk Eden Faiz (Min. Ödeme): ₺{(B0 * r_aylik).quantize(Decimal('.01'), rounding=ROUND_HALF_UP):,.2f}")
        except:
            self.lbl_dinamik_min.config(text="Tahakkuk Eden Faiz (Min. Ödeme): Bekleniyor...")

        if not hasattr(self, 'lbl_vade_min_odeme'): return
        try:
            if not self.b_vade_var.get() or int(self.b_vade.get()) < 1: raise ValueError
            B0 = Decimal(self.b_baslangic.get().replace('.', '').replace(',', '.'))
            F = Decimal(self.b_faiz.get()) / Decimal('100');
            vergi_orani_d = Decimal(self.b_vergi.get()) / Decimal('100')
            r_aylik = (F / Decimal('12')) * (Decimal('1') + vergi_orani_d)
    ...
    def _duzenli_islem_toggle(self):
        if self.m_duzenli_islem_var.get():
            ...
            self.m_yatirim_entry.pack_forget();
            self.m_yatirim.set("")
    ...
    def _mevduat_sekmesi(self, parent):
        ...
        self._etiket_giris(giris, "Stopaj (Vergi) (%):", "m_stopaj", 3, "yuzde",
                           "Aralık: %0.0 - %100.0\nFaizden kesilen yasal vergi (Örn: 5).")
        self._etiket_giris(giris, "Vade Günü (Tamsayı):", "m_vade_gun", 4, "tamsayi",
        ...
        self.btn_erken_ekle = tk.Button(erken_cekim_baslik, text="＋ Ekle", bg=r["ACCENT_RED"], fg=r["TEXT_PRIMARY"],
        ...
        tk.Checkbutton(self.enf_frame, text="Reel Alım Gücü (Enflasyon İskontosu):", variable=self.m_enflasyon_aktif,
        ...
        tk.Button(giris, text="  Hesapla  ", bg=r["ACCENT_BLUE"], fg="#FFFFFF", font=("Courier New", 11, "bold"),
                  relief="flat", cursor="hand2", command=self._mevduat_hesapla).grid(row=12, column=0, columnspan=2,
                                                                                     sticky="ew", pady=(10, 0))
    ...
    def _borc_sekmesi(self, parent):
        ...
        def _kredi_sekme_sec(tip_id, aktif_renk):
            ...
            if tip_id == "İhtiyaç Kredisi" or tip_id == "Taşıt Kredisi":
                self.b_vergi.set("30");
                self.entry_b_vergi.config(state="disabled", disabledbackground=r["DARK_BG"])
        ...
        for tip_id, text, renk in [("İhtiyaç Kredisi", "💸 İhtiyaç", r["ACCENT_GREEN"]),
                                   ...
                                   ("Özel Kredi", "🛠️ Özel", r["ACCENT_GOLD"])]:
        ...
        entry_b_bas = self._etiket_giris(giris, "Toplam Borç/Kredi (₺):", "b_baslangic", 2, "para_gorsel",
                                         "Aralık: Maks 10 Basamak\nToplam borç tutarını giriniz.")
        entry_b_bas.is_currency_formatted = True;
        entry_b_bas.bind('<KeyRelease>', self._para_formatla_event, add='+')

        self.entry_b_faiz = self._etiket_giris(giris, "Kredi Faiz Oranı (Yıllık %):", "b_faiz", 3, "yuzde",
                                               "Aralık: %0.0 - %100.0\nKredinin yıllık brüt faiz oranını giriniz.")
        self.entry_b_vergi = self._etiket_giris(giris, "Vergi (KKDF+BSMV %):", "b_vergi", 4, "yuzde",
                                                "Aralık: %0.0 - %100.0\nFaize eklenen yasal vergi oranını giriniz.")

        self.entry_b_odeme = self._etiket_giris(giris, "Aylık Ödeme (₺):", "b_odeme", 5, "para_gorsel",
        ...
        f_ara_btn.grid(row=6, column=0, columnspan=2, sticky="ew", pady=(6, 2))
        tk.Label(f_ara_btn, text="Ara Ödemeler:", bg=r["PANEL_BG"], fg=r["TEXT_PRIMARY"],
                 font=("Courier New", 11)).pack(side="left")
        tk.Button(f_ara_btn, text="＋ Ekle", bg=r["ACCENT_GREEN"], fg=r["DARK_BG"], relief="flat",
        ...
        self.b_ara_odeme_frame.grid(row=7, column=0, columnspan=2, sticky="ew", pady=(0, 4))

        self.b_vade_frame = tk.Frame(giris, bg=r["PANEL_BG"]);
        self.b_vade_frame.grid(row=8, column=0, columnspan=2, sticky="w", pady=4)
        ...
        self.lbl_dinamik_min.grid(row=9, column=0, columnspan=2, sticky="w", pady=(2, 0), padx=(0, 8))
        self.lbl_vade_min_odeme = tk.Label(giris, text="", bg=r["PANEL_BG"], fg=r["ACCENT_BLUE"],
                                           font=("Courier New", 10, "bold"))
        self.lbl_vade_min_odeme.grid(row=10, column=0, columnspan=2, sticky="w", pady=(0, 8), padx=(0, 8))

        tk.Button(giris, text="  Hesapla  ", bg=r["ACCENT_BLUE"], fg="#FFFFFF", font=("Courier New", 11, "bold"),
                  relief="flat", cursor="hand2", command=self._borc_hesapla).grid(row=11, column=0, columnspan=2,
                                                                                  sticky="ew", pady=(8, 0))
        ...
        _kredi_sekme_sec("İhtiyaç Kredisi", r["ACCENT_GREEN"])

    def _b_ara_odeme_ekle(self):
        r = self.renkler;
        ...
        tk.Entry(f, textvariable=t_var, width=10, bg=r["ENTRY_BG"], fg=r["TEXT_PRIMARY"], relief="solid").pack(
            side="left", padx=(2, 6))
        tk.Button(f, text="✕", bg=r["ACCENT_RED"], fg="#FFFFFF", relief="flat", font=("Courier New", 9, "bold"),
                  command=lambda: [self.b_ara_odemeler_liste.remove((d_var, t_var)), f.destroy()]).pack(side="left")
    ...
    def _tablo_secim_grafik(self, tree, canvas, mod):
        ...
        hedef_ay = idx + 1 if mod == "b" else idx
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  INLINE ERROR HANDLING (PM Sprint 1 - Task 2)
    # ══════════════════════════════════════════════════════════════════════
    ...
    def _dogrula(self, deger_str, min_val, max_val, ad, lbl_uyari=None, sessiz=False):
        try:
            ...
            if not sessiz and lbl_uyari:
                self._hata_ve_temizle(None, "Geçersiz Veri",
                                      f"'{ad}' alanına {min_val} ile {max_val} arası eksiksiz sayı girin.", lbl_uyari)
            return None
    ...
    def _dogrula_int(self, deger_str, min_val, max_val, ad, lbl_uyari=None, sessiz=False):
        try:
            ...
            if not sessiz and lbl_uyari:
                self._hata_ve_temizle(None, "Geçersiz Veri",
                                      f"'{ad}' alanına {min_val}-{max_val} arası geçerli TAM SAYI girin.", lbl_uyari)
            return None
    ...
    def _tarih_dogrula(self, tarih_str, lbl_uyari=None, sessiz=False):
        try:
            ...
            if not sessiz and lbl_uyari:
                self._hata_ve_temizle(None, "Tarih Hatası", "Tarihi GG.AA.YYYY formatında eksiksiz girin.", lbl_uyari)
            return None
    ...
    def _parse_ozel_islemler(self, lbl_uyari, sessiz):
        ...
        for donem_var, tutar_var in self.m_ozel_islemler_liste:
            ...
            if not d_str and not t_str: continue
            try:
                d = int(d_str);
                t = float(t_str)
                ...
                if not sessiz and lbl_uyari:
                    self._hata_ve_temizle(None, "Özel İşlem Hatası",
                                          "İşlem satırlarını doldurun (Dönem: tam sayı, Tutar: sayısal).", lbl_uyari)
                return None
        return islemler

    def _mevduat_hesapla(self, sessiz=False):
        r = self.renkler
        self.m_uyari_lbl.config(text="")  # Önceki uyarıları temizle
        lbl = self.m_uyari_lbl

        faiz_str = self.m_faiz.get()
        if not sessiz and faiz_str in ["", ".", "-"]:
            self._hata_ve_temizle(self.m_faiz, "Eksik Veri", "'Brüt Faiz Oranı' alanını doldurun.", lbl);
            return False
        faiz = self._dogrula(faiz_str, 0, 100, "Brüt Faiz", lbl, sessiz)
        if faiz is None: return False

        stopaj_str = self.m_stopaj.get()
        if not sessiz and stopaj_str in ["", ".", "-"]:
            self._hata_ve_temizle(self.m_stopaj, "Eksik Veri", "'Stopaj' alanını doldurun.", lbl);
            return False
        stopaj = self._dogrula(stopaj_str, 0, 100, "Stopaj", lbl, sessiz)
        if stopaj is None: return False

        vade_str = self.m_vade_gun.get()
        if not sessiz and vade_str == "":
            self._hata_ve_temizle(self.m_vade_gun, "Eksik Veri", "'Vade Günü' alanını doldurun.", lbl);
            return False
        vade = self._dogrula_int(vade_str, 1, 999, "Vade Günü", lbl, sessiz)
        if vade is None: return False

        sure_str = self.m_sure.get()
        if not sessiz and sure_str == "":
            self._hata_ve_temizle(self.m_sure, "Eksik Veri", "'Tekrar Edecek Dönem' alanını doldurun.", lbl);
            return False
        ...
        if not sessiz and baslangic_str in ["", ".", "-", "-."]:
            self._hata_ve_temizle(self.m_baslangic, "Hata", "'Başlangıç Bakiye' alanını eksiksiz doldurun.", lbl);
            return False
        try:
        ...
        if self.m_duzenli_islem_var.get():
            ...
            if not sessiz and yatirim_str in ["", ".", "-", "-."]:
                self._hata_ve_temizle(self.m_yatirim, "Hata", "'Düzenli İşlem' alanını doldurun.", lbl);
                return False
            try:
                P = float(yatirim_str)
            except:
        ...
        try:
            enf = float(enf_str) if self.m_enflasyon_aktif.get() and enf_str and enf_str not in [".", "-"] else 0.0
        except ValueError:
        ...
        if self.m_erken_cekim_var.get():
            for d_var, t_var in self.m_erken_cekim_liste:
                ...
                try:
                    ...
                    if not sessiz: self._hata_ve_temizle(None, "Hata", "Erken çekim satırlarını kontrol edin.", lbl)
                    return False
        ...
        metin = f"Son Bakiye Tarihi : {bitis}\nDizi Durumu       : {sonuc['karakteristik']}\nKritik Sınır P_min: ₺{p_min:,.2f} /Dönem\n\nSon Bakiye Tutar  : ₺{nom[-1]:,.2f}\n"
        ...
        if sonuc["vade_bozuldu"]:
            ...
            if len(vade_bozulanlar) > 4:
                donemler_str = f"{vade_bozulanlar[0]}, {vade_bozulanlar[1]}, {vade_bozulanlar[2]}... dahil toplam {len(vade_bozulanlar)} farklı"
            else:
                donemler_str = ", ".join(map(str, vade_bozulanlar))
        ...
        self._dinamik_yazdir(self.m_sonuc_lbl, metin, base_size=11)
    ...
    def _borc_hesapla(self, sessiz=False):
        r = self.renkler
        self.b_uyari_lbl.config(text="")  # Önceki uyarıları temizle
        lbl = self.b_uyari_lbl

        faiz_str = self.b_faiz.get()
        if not sessiz and faiz_str in ["", ".", "-"]:
            self._hata_ve_temizle(self.b_faiz, "Eksik Veri", "'Kredi Faiz Oranı' alanını doldurun.", lbl);
            return False
        faiz = self._dogrula(faiz_str, 0, 100, "Kredi Faiz Oranı", lbl, sessiz)
        if faiz is None: return False

        vergi_str = self.b_vergi.get()
        if not sessiz and vergi_str in ["", ".", "-"]:
            self._hata_ve_temizle(self.b_vergi, "Eksik Veri", "'Vergi (KKDF)' alanını doldurun.", lbl);
            return False
        ...
        if not sessiz and baslangic_str in ["", ".", "-"]:
            self._hata_ve_temizle(self.b_baslangic, "Eksik Veri", "'Toplam Borç' alanını eksiksiz doldurun.", lbl);
            return False
        try:
            A0 = float(baslangic_str)
            if A0 <= 0:
                if not sessiz: self._hata_ve_temizle(self.b_baslangic, "Hata",
                                                     "'Toplam Borç' sıfır veya negatif olamaz.", lbl)
                return False
        except ValueError:
            if not sessiz: self._hata_ve_temizle(self.b_baslangic, "Hata", "'Toplam Borç' alanını eksiksiz doldurun.",
                                                 lbl)
            return False

        odeme_str = self.b_odeme.get().replace('.', '').replace(',', '.')
        if not sessiz and odeme_str in ["", ".", "-"]:
            self._hata_ve_temizle(self.b_odeme, "Eksik Veri", "'Aylık Ödeme' alanını eksiksiz doldurun.", lbl);
            return False
        try:
            P = float(odeme_str)
        except ValueError:
        ...
        if self.b_vade_var.get():
            ...
            if not sessiz and vade_str == "":
                self._hata_ve_temizle(self.b_vade, "Eksik Veri", "'Vade Hedefi' alanına sayı girin.", lbl);
                return False
        ...
        for dv, tv in self.b_ara_odemeler_liste:
            if dv.get() and tv.get(): ara[int(dv.get())] = Decimal(tv.get())
        ...
        if sonuc["iraksar"]:
            ...
            if abs(float(P) - float(sonuc['p_min'])) < 0.01:
                metin = f"DİKKAT: Dizi {sonuc['karakteristik']}\nAylık ödemeniz (₺{P:,.2f}), tam olarak aylık faize eşit.\nSadece faiz ödendiği için anapara eksilmiyor ve borç bitmiyor.\n\nToplam Ödenen: ₺{sonuc['toplam_odenen_nom']:,.2f}"
            else:
                metin = f"DİKKAT: Dizi {sonuc['karakteristik']}\nAylık ödemeniz (₺{P:,.2f}), tahakkuk eden \nilk faizi (₺{sonuc['p_min']:,.2f}) karşılamıyor.\n\nToplam Ödenen: ₺{sonuc['toplam_odenen_nom']:,.2f}"
            ...
            metin = f"Borç Bitiş Tarihi : {bitis}\nDizi Durumu       : {sonuc['karakteristik']}\nToplam Ödenen     : ₺{sonuc['toplam_odenen_nom']:,.2f}\n"
        ...
        self._dinamik_yazdir(self.b_sonuc_lbl, metin, base_size=11)
        ...
        # Iraksak ise otomatik olarak logaritmik ölçeğe geçiş tetiklenir (PM Sprint 2 - Task 3)
